#!/usr/bin/env node
/**
 * assert-casey-fur.mjs — fail CI if any Casey PNG has become a black silhouette.
 *
 * A healthy Casey PNG has ≥8000 opaque white/cream pixels (the fur).
 * A corrupted one (flood-fill ate the fur) has almost none.
 *
 * Pure Node.js — no external image libs needed. Only checks 8-bit RGBA/RGB PNGs.
 */
...
// Thresholds — must have ≥ MIN_WHITE white-ish opaque pixels,
// and white count must be ≥ RATIO × dark count.
const MIN_WHITE = 8000;
const DARK_RATIO = 2;
...
function countFur(file) {
  const buf = readFileSync(file);
  const { width, height, bitDepth, colorType, idatParts } = parsePng(buf);
  if (bitDepth !== 8) return null; // skip unusual bit depths
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (a < 200) continue;
      if (r > 210 && g > 210 && b > 200) white++;
      if (r < 40 && g < 40 && b < 50) dark++;
    }
  }
  return { white, dark, width, height };
}
...
for (const tier of TIERS) {
  ...
  for (const file of files) {
    ...
    let result;
    try {
      result = countFur(fullPath);
    } catch (e) {
      errors.push(`${tier}/${file}: parse error — ${e.message}`);
      continue;
    }
    if (!result) continue; // non-8-bit or non-RGBA/RGB — skip

    const { white, dark } = result;
    const label = `${tier}/${file}`;
    if (white < MIN_WHITE) {
      errors.push(`${label}: only ${white} white pixels (need ≥${MIN_WHITE}) — possible black silhouette`);
    } else if (dark > 0 && white < dark * DARK_RATIO) {
      errors.push(`${label}: white=${white} < dark=${dark}×${DARK_RATIO} — fur may be corrupted`);
    }
  }
}

if (errors.length) {
  console.error('Casey fur check FAILED:');
...
if (totalPngs < MIN_PNG_INVENTORY) {
  console.error(`Casey fur check FAILED: only ${totalPngs} tier PNGs (need ≥${MIN_PNG_INVENTORY})`);
  process.exit(1);
}

console.log(`OK: ${totalPngs} Casey PNGs passed fur check across ${TIERS.length} tiers`);
