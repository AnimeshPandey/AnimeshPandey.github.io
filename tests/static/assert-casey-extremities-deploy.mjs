#!/usr/bin/env node
/**
 * assert-casey-extremities-deploy.mjs — mottled-extremity gate on shipped deploy bytes.
 * Mirrors assert-casey-extremities.mjs against _deploy/cases/assets/casey/.
 */
...
function scoreExtremities(file) {
  ...
  return { mottled, opaque, ratio: opaque ? mottled / opaque : 0 };
}
...
function checkFile(label, fullPath) {
  ...
  if (opaque > 200 && ratio > MAX_MOTTLE_RATIO) {
    errors.push(
      `${label}: mottle ratio ${(ratio * 100).toFixed(1)}% (${mottled}/${opaque}) — need ≤${(MAX_MOTTLE_RATIO * 100).toFixed(0)}%`
    );
  }
}
...
for (const tier of TIERS) {
  ...
  for (const file of files) {
    checkFile(`deployed ${tier}/${file}`, path.join(tierDir, file));
  }
}

const anchorDir = path.join(CASEY_DEPLOY, 'style-anchor');
if (existsSync(anchorDir)) {
  for (const file of readdirSync(anchorDir).filter((f) => f.endsWith('.png') && f.includes('-front'))) {
    checkFile(`deployed style-anchor/${file}`, path.join(anchorDir, file));
  }
}

if (errors.length) {
  console.error('Deployed Casey extremities check FAILED:');
  errors.forEach((e) => console.error('  ✗', e));
  process.exit(1);
}

console.log(`OK: ${checked.length} deployed Casey PNGs passed extremities mottle check`);
