#!/usr/bin/env node
/** assert-casey-floor-matte-deploy.mjs — floor matte guard on shipped hub/coach poses. */
...
for (const tier of TIERS) {
  for (const pose of POSES) {
    const fullPath = path.join(CASEY_DEPLOY, tier, pose);
    let result;
    try {
      result = countFloor(fullPath);
    } catch (e) {
      errors.push(`deployed ${tier}/${pose}: ${e.message}`);
      continue;
    }
    if (!result) continue;
    checked.push(`${tier}/${pose}`);
    if (result.floor > MAX_FLOOR) {
      errors.push(`deployed ${tier}/${pose}: ${result.floor} floor pixels (max ${MAX_FLOOR})`);
    }
  }
}

if (errors.length) {
  console.error('Deployed Casey floor matte check FAILED:');
